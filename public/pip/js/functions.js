...
/*
#######################################################################################################################################
// the class dedicated to deal with local database in browser where will save data in form of the same as the one in backend but there is no relation ship instead the developer decided to do it. the class will use cookies and datastructure( while a user is on the page ) to store information but in form of relational database.
#######################################################################################################################################
*/
class webApp{
   constructor(Database, host = "", password = "", username = "", struct = ""){
       ...
       if(struct.length>0) Structure = struct; else Structure = struct;
       // checking the list of existing database
...
// the admin class to save data in cookies with the same structure with that of the backend but it will use cookies

class admin extends webApp{
    constructor(argument, prim = "", conn = 0){
        this.name = argument;
        if(prim.length>0) this.primary_key = prim; else this.primary_key = `${this.name}_id`;
...
class PIP_DAT{
    ...
    left(DATE = "none"){
        ...
        var x = setInterval(function() {
          ...
          var finals = days + "day(s) " + hours + "h "
          + minutes + "m " + seconds + "s ";
          THIS.element().HTML(finals);
          // If the count down is finished, write some text
          if (distance < 0) {
            clearInterval(x);
            THIS.element().HTML("Expired!");
          }
        }, 1000);
    }
    day(DATE = "none"){
    ...
    _gets_(ind,val = "",type="REMOVE"){
       var DATS = [];
        if(parseInt(ind)){
            ...
            else {return this._gets_(this.fields[0],ind,type);}
        } else {
...
// a class to deal with html and DOM elements on the client side 
// ********** to be continued ************************
class _HTML {
    constructor(EL){
        this.theCurs = EL;
    }
    options(display,current = 0){
        var rets = "";
        var selected = "";
    }
}
// a class to act like PIP_Arr pointer or cursor
// ********** to be continued ************************
...
class _HTTP {
    constructor(file = "index", DEFAULT_TECH = ".php"){
        if(file.length>4){
            var prefx = file.substring(file.length-4,file.length);
            if(!(prefx==DEFAULT_TECH))
                file = file+DEFAULT_TECH;
            else file = file;
        }
        else file = file+DEFAULT_TECH;
        this.file = () =>{
            return file;
        }
        var params = [];
        this.par = function(name = null,value = null){
            if(name){
        ...
        this.serialize = () =>{
            var xc = "";
            for(var ii = 0; ii < this.par().length; ii++){
                xc = this.par()[ii].name+"="+this.par()[ii].value;
                if(xc<this.par().length()-1){
                    xc = xc+"&";
                }
            }
            if(xc.length==0)
        ...
        this.successFun = function(mess){
            console.log(mess);
        }
        this.failedFun = function(mess){
            console.log(mess);
        }
    ...
    post(){
        ...
        var THIS = this.checkProcess();
        
        this.req().open("POST",this.file(),true);
        this.req().setRequestHeader("Content-Type","application/x-www-form-urlencoded");
        this.req().onreadystatechange = function(){
            if(this.readyState == 4 && this.status == 200){
                THIS.successFun(this.responseText);
            } else if(this.readyState == 4){
                THIS.failedFun(THIS.createFailedMessage(this.status));
            }
        }
        try{
            this.req().send(this.encodeValue(this.serialize()));
        } catch(e){
            console.log(e);
        }
    ...
    get(){
        var THIS = this.checkProcess();
        this.req().open("GET",this.file(),true);
        this.req().setRequestHeader("Content-Type","application/x-www-form-urlencoded");
        this.req().onreadystatechange = function(){
            if(this.readyState == 4 && this.status == 200){
                THIS.successFun(this.responseText);
    ...
    createFailedMessage(status){
        // we have to handle all HTTP error code from the server
        switch(status){
            case 404:{
                return "Not found";
            }
            ...
            default:{
                return "Unkown Communication error";
            }   
        }
    }
    encodeValue(val){
...
function http(file = "index", tech = ".php"){
    return new _HTTP(file,tech);
}
class loadRealTime{
    constructor(file){
        this.url = () =>{
    ...
    load(divIds = "REALTIMEDIV"){
        $(this.theDiv).attr("id",divIds);
        $("body").append(this.theDiv);
        return this;
    }
    
}
// the class that will handle the browser information to the server the same as the one in php
class browser{
    constructor(){
        this.height;
        this.width;
